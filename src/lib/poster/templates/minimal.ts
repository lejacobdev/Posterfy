...
export function renderMinimal(rc: RenderContext, locale: string): void {
  ...
  drawArtwork(rc, coverX, cursorY, coverSize, coverSize);
  ...
  if (spec.options.paletteStyle === 'dots' && theme.palette.length > 0) {
    ...
    drawPaletteDots(rc, centerX - totalWidth / 2, cursorY, dotSize, dotSize * 0.55);
    cursorY += dotSize + gap;
    ...
    theme.palette.forEach((color, index) => {
      ctx.fillStyle = color;
      ctx.fillRect(centerX - stripWidth / 2 + index * step, cursorY, step + 0.5, stripHeight);
    });
    ctx.restore();
  ...
  if (entries.length > 0) {
    ...
    drawText(ctx, line, centerX, cursorY + metaSize, labelFont(rc, metaSize));
    ctx.restore();
  ...
  if (hasTracklist) {
    ...
    drawTracklist(rc, {
      ...
      y: cursorY,
      width: box.width * 0.8,
      maxHeight: Math.max(0, box.y + box.height - cursorY - bottomReserve),
  ...
  if (spec.options.showScanCode) {
    ...
    drawScanBlock(rc, {
      ...
      y: box.y + box.height - metaSize * 2.2,
      width: scanWidth,
      height: metaSize * 1.4,
    });
  }
